...
let OGRS4G = {
  getOGRS4GStaticScore(x) {
    return 0 +
      ...
      // x.coefficients.male[0] | female[0]

      //A quartic equation which uses the offenders age at the Date of discharge/sentencing of a community order
      (x.ageIndex * lookupParameter(x.gender, 'AgeIndexFactor')[1]) +
        (Math.pow(x.ageIndex, 2) * lookupParameter(x.gender, 'AgeIndexFactor')[2]) +
          (Math.pow(x.ageIndex, 3) * lookupParameter(x.gender, 'AgeIndexFactor')[3]) +
            (Math.pow(x.ageIndex, 4) * lookupParameter(x.gender, 'AgeIndexFactor')[4]) +
       // x.coefficients.male[1-4] | female[1-5]

      //Uses the type of offence the offender has commited
      lookupParameter(x.typeOfCurrentOffence, 'OffenceGroup') +
      // x.coefficients.offenceCategoriesLookup

      //Score depending on whether the offender is a first, second or repeat offender
      lookupParameter(x.careerStatus, 'CareerStatus') +
      // x.coefficients.firstSanction | x.coefficients.secondSanctionMale | x.coefficients.secondSanctionFemale
      ...
      // x.coefficients.sanctionOccasions
      ...
      // x.coefficients.criminalCareerMale | x.coefficients.criminalCareerfemale

      //Score changes depending on the copas score
      (Math.pow(x.copasScoreGen, 1) * lookupParameter(x.careerStatus, 'CopasFactor')) +
        (Math.pow(x.copasScoreGen, 2) * lookupParameter(x.careerStatus, 'CopasFactor')) +
      // x.coefficients.copasMale | x.coefficients.copasFemale

      //A quartic equations which uses the number of offence free months
      (Math.pow(x.offFree, 1) * getRange('OffenceFreeFactor')[0]) +
        (Math.pow(x.offFree, 2) * getRange('OffenceFreeFactor')[1]) +
          (Math.pow(x.offFree, 3) * getRange('OffenceFreeFactor')[2]) +
            (Math.pow(x.offFree, 4) * getRange('OffenceFreeFactor')[3]);
      // x.coefficients.offenceFreeMonths[0-3]
  },
...
let SVStatic = {
  getSVStatic(x) {
    ...
    let Score = 0 +
      ...
      //if the offender has 2 or more previous offences then use the rate of offending
      (
        x.previousSanctions > 1 ?
          x.VcopasScoreGen * lookupParameter(x.gender, 'SVStaticThirdOGRS4V') : 0
      ) +

      //if the offender has no previous violent offences then add a constant to the score
      (
        x.previousSanctionsForViolentOffences === 0 ?
          lookupParameter(x.gender, 'SVStaticViolenceHistory') : 0
      ) +

      //if the offender has one previous violent offences then add a constant to the score
      (
        x.previousSanctionsForViolentOffences === 1 ?
          lookupParameter(x.gender, 'SVStaticViolenceHistory') : 0
      ) +
      ...
      //if the current offence is VATP then add a constant depending on what category it is
      (
