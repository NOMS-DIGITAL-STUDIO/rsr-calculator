...
const DateTime = {
  ...
  getYearDiff(start, end) {
    if (DateTime.getMonth(end) > DateTime.getMonth(start)) {
      return DateTime.getYear(end) - DateTime.getYear(start);
    }

    if (DateTime.getMonth(end) < DateTime.getMonth(start)) {
      return DateTime.getYear(end) - DateTime.getYear(start) - 1;
    }

    if (DateTime.getDay(end) >= DateTime.getDay(start)) {
      return DateTime.getYear(end) - DateTime.getYear(start);
    }
  ...
  getMonthDiff(start, end) {
    return getYearDiff(start, end) * 12 +
      // need to add on actual month diff
      0;
  }
};

const calculateCareerStatus = x => {
  if (x.numberOfPreviousSanctions === 0) {
    return 'First-time entrant';
  }

  if (x.numberOfPreviousSanctions === 1) {
...
const calculateCareerStatusViolent = x => {
  if (x.numberOfPreviousSanctionsForViolentOffences === 0) {
    return 'Never violent (' + x.gender + ')';
  }

  if (x.numberOfPreviousSanctionsForViolentOffences === 1) {
    return 'Once violent';
  }
...
let StoreInputs = {
  getOtherVariables(x) {
    if (x.offenderHasSexualOffenceHistory) {
      x.ageSO = DateTime.getYearDiff(x.dateOfBirth, x.dateOfMostRecentSexualOffence);
    }

    Object.assign(x, {
      ageRisk: DateTime.getYearDiff(x.dateOfBirth, x.dateOfSentenceOrDischarge),
    ...
    Object.assign(x, {
      ...
      copasScoreGen: Math.log((1 + x.numberOfPreviousSanctions) / (getRange('OGRS4GCopasDenom') + x.ageCurSanc - x.ageFirSanc)),
      VcopasScoreGen: Math.log((1 + x.numberOfPreviousSanctions) / (getRange('OGRS4VCopasDenom') + x.ageCurSanc - x.ageFirSanc)),
    });

    x.VcopasScoreViolent = (x.numberOfPreviousSanctionsForViolentOffences > 0) ?
      Math.log((x.numberOfPreviousSanctionsForViolentOffences) / (getRange('OGRS4VCopasDenomViolent') + x.ageCurSanc - x.ageFirSanc)) :0;

    Object.assign(x, {
      careerStatus: calculateCareerStatus(x),
      careerStatusViolent: calculateCareerStatusViolent(x),

      offenderHasSexualOffenceHistory: (x.numberOfSanctionsInvolvingContactAdultSexualOffences +
                                        x.numberOfSanctionsInvolvingContactChildSexualOffences +
                                        x.numberOfSanctionsInvolvingIndecentImageSexualOffences +
                                        x.numberOfSanctionsInvolvingParaphiliaSexualOffences > 0),
...
let OSP = {
  getOSPStaticScore(x) {
    return 0 +
      ...
      lookupParameter(x.numberOfSanctionsInvolvingContactAdultSexualOffences, 'AdultSO') +
      lookupParameter(x.numberOfSanctionsInvolvingContactChildSexualOffences, 'ChildSO') +
      lookupParameter(x.numberOfSanctionsInvolvingParaphiliaSexualOffences, 'ParaphiliaSO') +
      lookupParameter(x.numberOfPreviousSanctions, 'PreviousSanctions') +

      //Add to the score if the victim was a stranger
      lookupParameter(currentOffenceHasStrangerVictim, 'StrangerSO') +

      //Add to the score depending on the age of most recent sexual offence
      lookupParameter(ageSO, 'AgeSO') +

      //Add to the score depending on what age the offender will be on their Date of discharge/sentencing of a community order
      lookupParameter(ageRisk, 'AgeRisk');
  },

  getOSP(x) {
    // if the offender is male with has sexual offence history
...
let OGRS4G = {
  getOGRS4GStaticScore(x) {
    return 0 +
      ...
      ((x.numberOfPreviousSanctions + 1) * getRange('SanctionsFactor')) +
...
let OGRS4V = {
  getOGRS4VStaticScore(x) {
    return 0 +
      ...
      //Score depending on the number of previous sanctions
      //Add 1 to include the current offence
      ((x.numberOfPreviousSanctions + 1) * getRange('VSanctionsFactor')) +
      ...
      (x.numberOfPreviousSanctionsForViolentOffences * getRange('VSanctionsFactorViolent')) +
      ...
      (
        x.numberOfPreviousSanctionsForViolentOffences > 0 ?
...
let OGP2 = {
  getOGP2StaticScore(x) {
    return 0 +
      ...
      lookupParameter(x.numberOfPreviousSanctions, 'OGP2Sanctions') +

      //Multiply the number of convictions by a factor and add to the score
      //Add 1 to include the current offence
      (x.numberOfPreviousSanctions + 1 * getRange('OGP2SanctionFactor')) +

      //if it is the offenders second offence add a constant to the score
      (
        x.numberOfPreviousSanctions === 1 ?
...
let OVP2 = {
  getOVP2StaticScore(x) {
    return 0 +
      ...
      //Add a constant depending on whether the offender has previous sanctions
      lookupParameter(x.numberOfPreviousSanctions, 'OVP2Sanctions')(2, true) +

      //Multiply the number of convictions by a factor and add to the score
      //Add 1 to include the current offence
      ((x.numberOfPreviousSanctions + 1) * getRange('OVP2SanctionFactor')) +
      ...
      (
        x.numberOfPreviousSanctionsForViolentOffences === 1 ?
          lookupParameter(x.gender, 'OVP2ViolenceHistory')(3, false) : 0
      ) +

      //Multiply the number of violent offences by a factor and add to the score
      (x.numberOfPreviousSanctionsForViolentOffences * getRange('OVP2ViolentPreCons'))
...
let IndecentImages = {
  ...
  calculateIndecentImageProbability(x) {
    ...
    if (x.numberOfSanctionsInvolvingIndecentImageSexualOffences > 1) {
    ...
    if (x.numberOfSanctionsInvolvingContactChildSexualOffences > 1) {
    ...
    if (x.numberOfSanctionsInvolvingContactAdultSexualOffences +
          x.numberOfSanctionsInvolvingParaphiliaSexualOffences > 0) {
...
let SVStatic = {
  getSVStatic(x) {
    ...
    let Score = 0 +
      ...
      //Add a constant depending on whether the offender has previous sanctions
      lookupParameter(x.numberOfPreviousSanctions, 'SVStaticSanctions') +

      //Add 1 to include the current offence
      //Add to the score depending on how many sanctions the offender has
      //Cap the previous convictions at 50
      (Math.min(x.numberOfPreviousSanctions, 50) + 1) * getRange('SVStaticSanctionFactor') +

      //Add to the score if the offender has 1 previous offence depending on the time between the first offence and the current one
      (
        numberOfPreviousSanctions === 1 ?
      ...
      (x.numberOfPreviousSanctionsForViolentOffences * getRange('SVStaticViolentPreCons')) +
  ...
  getSVDynamic(x) {
    if (x.offenderHasHadAnOffenderInterviewOrOASys) {
      let Score = 0 +
        ...
        //Add a constant depending on whether the offender has previous sanctions
        lookupParameter(x.numberOfPreviousSanctions, 'SVDynamicSanctions') +

        //Add 1 to include the current offence
        //Cap the previous convictions at 50
        ((Math.min(x.numberOfPreviousSanctions, 50) + 1) * getRange('SVDynamicSanctionFactor')) +
        ...
        //Add a constant depending on the the number of previous violent offences
        (x.numberOfPreviousSanctionsForViolentOffences * getRange('SVDynamicViolentPreCons')) +
...
x.currentOffenceHasStrangerVictim = false;
x.categoryOfViolenceOffence = '';

//Criminal History
x.dateOfFirstSanction = new Date();
x.numberOfPreviousSanctions = 0;
x.numberOfPreviousSanctionsForViolentOffences = 0;
x.offenderHasCriminalHistory = false;
x.dateOfMostRecentSexualOffence = new Date();
x.numberOfSanctionsInvolvingContactAdultSexualOffences = 0;
x.numberOfSanctionsInvolvingContactChildSexualOffences = 0;
x.numberOfSanctionsInvolvingIndecentImageSexualOffences = 0;
x.numberOfSanctionsInvolvingParaphiliaSexualOffences = 0;
