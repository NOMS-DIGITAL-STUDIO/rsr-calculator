...
const calcCriminalCareerStatusFactor = (x) => {
  switch(x.allSanctions) {
    ...
    default:  // 3+ total sanctions
      let copasScoreGeneral = Math.log(x.allSanctions / ((x.ageAtConvictionDate - x.ageAtFirstSanctionDate) + 26));
...
const calculateOGRS4GStaticScore = x =>
  x.ageFactor +
...
module.exports = x => {
  let y = {
    coefficients: x.coefficients.OGRS4G,
  ...
  y.staticScore = calculateOGRS4GStaticScore(y);

  return calculateOGRS4G(y);
};
