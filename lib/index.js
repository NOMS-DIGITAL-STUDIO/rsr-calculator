...
const withAppropriateCoefficients = (fn) => (x) => {
  var out = {};
  for (var c in Coefficients) {
    switch (c) {
      case 'offenceCategoriesLookup':
      case 'vatpLookup':
        out[c] = Coefficients[c].map((y) => y[x.oasysInterview]);
        break;
      default:
        out[c] = Coefficients[c][x.oasysInterview];
    }
  }
  x.coefficients = out;
  return fn(x);
...
const calcOGRS4sRiskBand = (x) => {
  if (x.OGRS4s < 12) return 'Low';
  if (x.OGRS4s < 15) return 'Medium';
  if (x.OGRS4s < 18) return 'High';
  return 'Very high';
}

const calcRSRPercentileRisk = (x) =>
  x.riskOfSeriousRecidivism.map((score) => 1 * parseFloat(Math.round(score * 10000) / 100).toFixed(2));

const calcRSRRiskBand = (x) =>
  x.riskOfSeriousRecidivism.map((p) => {
    if (p < 0.03) return 'Low';
    if (p < 0.069) return 'Medium';
    return 'High';
  });

const calculateRisk = (x) => [
    ['OGRS3', withAppropriateCoefficients(calcOGRS3)],
    ['OGRS4s', calcOGRS4s],
    ['OGRS4sRiskBand', calcOGRS4sRiskBand],
    ['probabilityOfNonSexualViolence', withCoefficients(calcProbabilityOfNonSexualViolence)],
