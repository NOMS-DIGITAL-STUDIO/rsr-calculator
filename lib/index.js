...
// const withValidOSPInputs = (x, fn) => {
//   //TODO: Some validation
//   return fn(x)
// }

const calculateOSP = (x) =>
  collect([
    ['sexualContactProbability', calculateOffenderSexualProbability],
    ['indecentImageProbability', calculateIndecentImageProbability]
  ])(x)
...
const transformOSPInput = (x) => {
  return {
    isMale: x.sex === 'MALE',
    hasSexualHistory: x.hasSexualHistory,
    ageAtMostRecentSexualOffence: yearDiff(x.dateOfBirth, x.mostRecentSexualOffenceDate),
    ageAtRiskDate: yearDiff(x.dateOfBirth, x.sentenceDate),
    // current sexual offence has a stranger victim
    hasStrangerVictim: x.hasStrangerVictim,
    // Number of previous sanctions for indecent images
    indecentImage: x.numberOfSanctionsIndecentImages,
    // Number of previous sanctions for all recordable offences
    previousSanctions: x.numberOfPreviousSanctions,
    // Number of previous/ current sanctions involving contact child sexual offences
    contactChild: x.numberOfSanctionsChildContact,
    // Number of previous/ current sanctions involving contact adult sexual offences
    contactAdult: x.numberOfSanctionsAdultContact,
    // Number of previous / current sanctions involving paraphilia sexual offences
    paraphilia: x.numberOfSanctionsParaphilia,
...
const xyz = (x) => {
  return calculateOSP(withOSPStaticScore(transformOSPInput(x)));
}

const withOSPStaticScore = (x) => {
...
module.exports = {
  ...
  calculateOSP: withCalculatorVersion(xyz),
  // calculateOSP: withCalculatorVersion(calculateOSP),
