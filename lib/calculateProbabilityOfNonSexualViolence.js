...
const calcSecondSanctionFactor = (x) =>
  x.coefficients.secondsanction + calcYearsSinceFirstSanction(x) *
    sexBasedFactor(x.coefficients.malesecondsanctionyearssincefirs, x.coefficients.femalesecondsanctionyearssincefi, x);
const calcAdditionalSanctionFactor = (x) =>
  Math.log(calcSanctionOccasions(x) / (calcYearsSinceFirstSanction(x) + 12)) *
    sexBasedFactor(x.coefficients.malethreeplussanctionsogrs4v_rat, x.coefficients.femalethreeplussanctionsogrs4v_r, x);
...
const calcOGRS4v = (x) =>
  ((x.violentSanctions * x.coefficients.ogrs3_ovp_sanct) +
  (x.violentSanctions === 1 ? x.coefficients.onceviolent : 0) +
  Math.log(x.violentSanctions / (calcYearsSinceFirstSanction(x) + 30)) * x.coefficients.ogrs4v_rate_violent_parameter_estimate);

const calcWithNoViolentSanctionsFactor = (x) =>
  sexBasedFactor(x.coefficients.maleneverviolent, x.coefficients.femaleneverviolent, x);
