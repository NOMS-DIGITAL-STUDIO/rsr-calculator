...
function getOASysScore(x) {
  var hasOASys = parseInt(x.oasysInterview, 10) === 0;
...
function getNonSexualViolenceProbability(x) {
  ...
  var z1 = Math.exp(Intercept_1[s] + x.score);
  var z2 = Math.exp(Intercept_2[s] + x.score);

  return [ z1 / (1 + z1), z2 / (1 + z2) ];
}

function getIndecentImageProbability(x) {
  var isMale = (parseInt(x.sex, 10) === 0);
  var hasSexualHistory = parseInt(x.sexualOffenceHistory, 10) === 0 || parseInt(x.sexualElement, 10) === 0;

  if (!isMale || !hasSexualHistory) {
    return [ 0, 0 ];
  }

  if (parseInt(x.indecentImage, 10) + parseInt(x.contactChild, 10) + parseInt(x.contactAdult, 10) + parseInt(x.paraphilia, 10) === 0) {
    return [ 0.000098, 0.000152 ];
  } else if (parseInt(x.indecentImage) > 1) {
    return [ 0.038087, 0.057949 ];
  } else if (parseInt(x.indecentImage) === 1) {
    return [ 0.018237, 0.02805 ];
  } else if (parseInt(x.contactChild) > 1) {
...
function getOSPContactAdultScore(ca) {
  if (parseInt(ca) === 0) {
    return 0;
  }
  if (parseInt(ca) === 1) {
...
function getOSPContactChildScore(cc) {
  if (parseInt(cc) === 0) {
    return 0;
  }
  if (parseInt(cc) === 1) {
...
function getOSPParaphiliaScore(p) {
  if (parseInt(p) === 0) {
    return 0;
  }
  if (parseInt(p) === 1) {
...
function getOSPAgeAtMostRecentSexOffenceScore(a) {
  if (parseInt(a) > 17) {
    return 5;
  }
  if (parseInt(a) > 15) {
...
function getOSPAgeAtRiskDateScore(a) {
...
function getOSPScore(x) {
  var ageAtMostRecentSexOffence = dateDiff(x.mostRecentSexualOffence, x.birthDate);
  var ageAtRiskDate = dateDiff(x.sentenceDate, x.birthDate);

  return 0 +
    getOSPContactAdultScore(x.contactAdult) +
    getOSPContactChildScore(x.contactChild) +
    getOSPParaphiliaScore(x.paraphilia) +
    (parseInt(x.previousSanctions) > 0 ? 3 : 0) +
    (parseInt(x.strangerVictim) === 0 ? 2 : 0) +
    getOSPAgeAtMostRecentSexOffenceScore(ageAtMostRecentSexOffence) +
    getOSPAgeAtRiskDateScore(ageAtRiskDate);
}

function getContactSexualProbability(x) {
  var isMale = (parseInt(x.sex, 10) === 0);
  var hasSexualHistory = parseInt(x.sexualOffenceHistory, 10) === 0 || parseInt(x.sexualElement, 10) === 0;
  ...
  if (isMale) {
    var OSPFactor = 0.2545385404 * getOSPScore({
      sex: x.sex,
    ...
    var z1 = Math.exp(-8.817398615 + OSPFactor);
    var z2 = Math.exp(-8.216503478 + OSPFactor);
...
function dateDiff(d1, d2) {
  ...
  if (d1.getMonth() - d2.getMonth() < 0 ||
        (d1.getMonth() - d2.getMonth() === 0 && d1.getDate() - d2.getDate() < 0)) {
    result--;
  }
...
function monthDiff(d1, d2) {
  return parseInt(((d1.getFullYear() - d2.getFullYear()) * 12) + (d1.getMonth() - d2.getMonth()), 10);
}
...
function calculateScore(o) {
  ...
  result.indecentImageProbability = getIndecentImageProbability({
    ...
    sexualOffenceHistory: o.sexualOffenceHistory,
    sexualElement: o.sexualElement
  });

  result.contactSexualProbability = getContactSexualProbability({
    sex: o.sex,
    birthDate: o.birthDate,
    mostRecentSexualOffence: o.mostRecentSexualOffence,
    sentenceDate: o.sentenceDate,
    contactAdult: o.contactAdult,
    contactChild: o.contactChild,
    paraphilia: o.paraphilia,
  ...
  result.totalRSR = [
    result.nonSexualViolenceProbability[0] + result.indecentImageProbability[0] + result.contactSexualProbability[0],
    result.nonSexualViolenceProbability[1] + result.indecentImageProbability[1] + result.contactSexualProbability[1]
  ];
