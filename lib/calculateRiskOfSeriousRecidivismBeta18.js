...
const getOASysScore = require('./calculateOASysPolynomial');
...
function getSanctionsPolynominal(x) {
  ...
  if (allSanctions === 2) {
    var ageAtSanction = yearDiff(x.convictionDate, x.birthDate);
    var ageAtFirstSanction = yearDiff(x.firstSanctionDate, x.birthDate);
...
function getAgePolynominal(x) {
  var s = parseInt(x.oasysInterview, 10);
  var isMale = (parseInt(x.sex, 10) === 0);
  var ageAtRiskDate = yearDiff(x.sentenceDate, x.birthDate);
...
function calculateScore(o) {
  ...
  result.OSPStaticScore = getOSPStaticScore({
    isMale: (parseInt(o.sex, 10) === 0),
    hasSexualHistory: parseInt(o.sexualOffenceHistory, 10) === 0 || parseInt(o.sexualElement, 10) === 0,
    ageAtMostRecentSexOffence: yearDiff(o.mostRecentSexualOffence || o.birthDate, o.birthDate),
    ageAtRiskDate: yearDiff(o.sentenceDate, o.birthDate),
